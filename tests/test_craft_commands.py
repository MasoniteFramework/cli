...
def test_craft_controller():
    ...
    subprocess.Popen.wait(command)
    assert os.path.exists('testnew/app/http/controllers/TestController.py')
...
def test_craft_model():
    ...
    subprocess.Popen.wait(command)
    assert os.path.exists('testnew/app/TestModel.py')
